...
const generateRandomCandidate = () => {
    const randomName = faker_1.faker.person.fullName();
...
app.put('/api/candidates/:id', (req, res) => {
    ...
    candidates[candidateIndex] = Object.assign(Object.assign({}, candidates[candidateIndex]), { name,
        description,
        party, image: image || `https://picsum.photos/200/200?random=${Date.now()}` });
...
app.get('/api/statistics', (req, res) => {
    ...
    const longestDescription = candidates.reduce((longest, candidate) => candidate.description.length > longest.length ? candidate.description : longest, '');
    const shortestDescription = candidates.reduce((shortest, candidate) => candidate.description.length < shortest.length || shortest === '' ? candidate.description : shortest, '');
    // Candidates with longest and shortest names
    const longestName = candidates.reduce((longest, candidate) => candidate.name.length > longest.length ? candidate.name : longest, '');
    const shortestName = candidates.reduce((shortest, candidate) => candidate.name.length < shortest.length || shortest === '' ? candidate.name : shortest, '');
    const statistics = {
        totalCandidates,
